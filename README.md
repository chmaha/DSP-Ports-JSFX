...
Copy the plugin's `.jsfx` file into your REAPER `Effects` folder (**Options → Show REAPER resource path…**). For Synthy McSynthface and Jexed, also copy the `Dependencies` folder that sits next to the `.jsfx`, and keep it next to the `.jsfx`.
